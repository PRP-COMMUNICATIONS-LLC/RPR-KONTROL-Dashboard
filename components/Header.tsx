...
const Header: React.FC<HeaderProps> = ({ classification }) => {
  const isCrownSecret = classification === Classification.TS_LAMBDA_3;

  return (
    <header className="bg-rpr-black text-rpr-white p-4 shadow-md sticky top-0 z-10 border-b border-rpr-charcoal">
      <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between">
        <div className="flex items-center gap-3 mb-2 sm:mb-0">
          ...
          <span className="font-extrabold tracking-wider text-xl" style={{ letterSpacing: '0.15em' }}>
            RPR COMMUNICATIONS
          </span>
        </div>
        <div 
          className={`text-sm font-medium px-3 py-1 rounded-full text-rpr-white border ${isCrownSecret ? 'border-rpr-cyan bg-rpr-black shadow-cyan-glow' : 'border-rpr-slate bg-rpr-charcoal'}`}
