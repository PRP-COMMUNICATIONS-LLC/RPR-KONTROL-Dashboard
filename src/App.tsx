 ...
 * Update: Bottom-Rail Navigation (MOTHERSHIP-V4)
 */

const firebaseConfig = {
  apiKey: import.meta.env.VITE_FIREBASE_API_KEY,
  authDomain: import.meta.env.VITE_FIREBASE_AUTH_DOMAIN,
  projectId: import.meta.env.VITE_FIREBASE_PROJECT_ID || "rpr-myaudit",
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A] text-slate-200 flex flex-col font-sans selection:bg-[#00D9FF]/30 overflow-x-hidden">
      {/* OVERWATCH HEADER - Fixed Top */}
      <header className="fixed top-0 left-0 right-0 p-4 md:p-8 flex items-center justify-between border-b border-white/5 bg-[#111622]/80 backdrop-blur-xl z-50">
        <RprKontrolLogo size="md" showSubtitle={true} />
      ...
      {/* FORENSIC VIEWPORT - Main Content with spacing for header and bottom rail */}
      <main className="flex-1 mt-16 pb-32 p-4 md:p-12 max-w-7xl mx-auto w-full">
          ...
          <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700">
             <div className="bg-[#16191E]/40 border border-white/5 p-6 md:p-10 rounded-2xl md:rounded-[3rem] shadow-2xl">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h3 className="text-[#00D9FF] text-[10px] font-black uppercase tracking-[0.3em] leading-none">// LIVE SIGNALS</h3>
                </div>
        ...
        {activeTab === 'icam' && (
          <div className="flex flex-col items-center justify-center py-12 md:py-20 bg-[#16191E]/40 border border-white/5 rounded-[2rem] md:rounded-[4rem] text-center space-y-6 md:space-y-8 animate-in zoom-in-95 duration-500">
            <div className="w-16 h-16 md:w-24 md:h-24 bg-[#00D9FF]/10 rounded-2xl md:rounded-[2rem] flex items-center justify-center text-[#00D9FF]">
              <Fingerprint size={32} className="md:w-12 md:h-12" />
            </div>
            <h3 className="text-xl md:text-2xl font-bold text-white uppercase tracking-tight">ICAM Module</h3>
            <p className="text-sm md:text-base text-slate-400 max-w-md">
              Identity & Access Management - Module initialization pending
            </p>
          </div>
        )}
      </main>

      {/* NAVIGATION RAIL - Fixed Bottom (5-Pillar Touchpoints) */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#05070A]/80 backdrop-blur-xl border-t border-white/5">
        <div className="p-3 md:p-4 flex justify-center items-center gap-2 md:gap-3 max-w-7xl mx-auto">
          {[
            { id: 'firm', label: 'THE FIRM', icon: Building },
            { id: 'live', label: 'LIVE VIEW', icon: Eye },
            { id: 'vault', label: 'THE VAULT', icon: FolderLock },
            { id: 'sessions', label: 'THE SESSIONS', icon: History },
            { id: 'icam', label: 'ICAM', icon: Fingerprint }
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex flex-col items-center justify-center gap-1 transition-all text-[8px] md:text-[9px] font-black tracking-widest uppercase px-3 py-2 md:px-4 md:py-3 rounded-xl border min-w-[60px] md:min-w-[80px] ${getTabStyle(tab.id)}`}
            >
